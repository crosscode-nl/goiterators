...
type GeneratorFunc[T any] func(p T, c, r uint64) T
...
type GeneratingIterator[T any] struct {
	...
	// previous contains the previous generated value
	previous T
}
...
func (g *GeneratingIterator[T]) Next() (T, bool) {
	...
	var r bool
	if g.count < g.repeat {
		g.previous = g.generator(g.previous, g.count, g.repeat)
		t = g.previous
		r = true
		g.count++
	}
...
func Generate[T any](p T, r uint64, gf GeneratorFunc[T]) *GeneratingIterator[T] {
...
func RepeatingIntegerGenerator[T SignedIntegers](i T, r uint64, s T) *GeneratingIterator[T] {
	...
	next := func(p T, c uint64, r uint64) T {
		return i + (s * T(c))
	}
	var t T
	return Generate(t, r, next)
}
